		...
		if (mysql_num_rows ($userquery) > 0){
			...
			<?php
				while ($userdata = mysql_fetch_array ($userquery)){
					?><div onmouseover="this.style.background = '#EEEEEE'" onmouseout="this.style.background = '#CCCCCC'" onclick="setvalueobat ('<?php echo $userdata['nama_obat'];?>')"><?php echo $userdata['nama_obat']; ?></div><?php
				}
			?>
			</div>
			<?php
