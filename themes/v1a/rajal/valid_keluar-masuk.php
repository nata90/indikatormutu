...
if($_POST['Keluar']){
   ...
   if($cek_keluar == 1){
	...
	if($status == "5"){
		   ...
		   $sqlsearchpendaftaran = "select NOMR, TGLREG, KDPOLY, KDCARABAYAR
		   							from t_pendaftaran 
			...
			if(mysql_num_rows($rowpendaftaran) > 0){
				...
				if($d['KDPOLY'] == $_REQUEST['poly']){
					...
					if($d['KDCARABAYAR'] == 1){
						$sqlinsert_pendaftaran = "INSERT INTO t_pendaftaran (NOMR, TGLREG, KDDOKTER, KDPOLY, KDRUJUK, KDCARABAYAR, NOJAMINAN, SHIFT, `STATUS`, PASIENBARU, NIP)  VALUES('".$nomr."',curdate(),".$kddokter.",".$kdpoly.",".$kdrujuk.",".$kdcarabayar.",'".$nojaminan."',".$shift.",0,0,'".$nip."')";
