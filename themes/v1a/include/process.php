...
if(!empty($_GET['psn'])){
	...
	if($lihat == $data['NOMR']){
	  ...
	  echo 	$r.'|'.
			...
			$data['NOTELP'].'|'.
			$data['NOKTP'].'|'.
			$data['SUAMI_ORTU'].'|'.$data['PEKERJAAN'].'|'.$data['STATUS'].'|'.$data['AGAMA'].'|'.$data['PENDIDIKAN'].'|'.$data['KDCARABAYAR'].'|'.$data['ALAMAT_KTP'].'|'.$umur.'|'.$title.'|'.$data['PENANGGUNGJAWAB_NAMA'].'|'.$data['PENANGGUNGJAWAB_HUBUNGAN'].'|'.$data['PENANGGUNGJAWAB_ALAMAT'].'|'.$data['PENANGGUNGJAWAB_PHONE'];
	}else{
	  $r 	= 1;
...
if($_REQUEST['idxb']){
	...
	while($dt = mysql_fetch_array($tcount)){
		if($sisa > 0): # Jika keringanan > 0
			...
			if($sisa_s > 0):
				mysql_query("UPDATE t_billrajal SET SHIFT='".$_POST['SHIFT']."', COSTSHARING = '".$dt['TOTAL']."' WHERE IDXBILL='".$dt['IDXBILL']."'");
			elseif($sisa_s < 0):
				mysql_query("UPDATE t_billrajal SET SHIFT='".$_POST['SHIFT']."', COSTSHARING = '".$sisa."' WHERE IDXBILL='".$dt['IDXBILL']."'");
			else:
				mysql_query("UPDATE t_billrajal SET SHIFT='".$_POST['SHIFT']."', COSTSHARING = '".$dt['TOTAL']."' WHERE IDXBILL='".$dt['IDXBILL']."'");
			endif;
			$sisa = $sisa_s;
