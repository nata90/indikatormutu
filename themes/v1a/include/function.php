...
function getLastNoM($p=0){
	$sql="select NOMR as nomor from m_pasien order by NOMR desc limit 1";
	$query=mysql_query($sql);
	$data=mysql_fetch_assoc($query);
	$v = $data['nomor'] + $p;
	$vl=strlen($v);
	if($v < 10){ $value = '00000'.$v;
	}else if($v < 100){ $value = '0000'.$v;
	}else if($v < 1000){ $value = '000'.$v;
	}else if($v < 10000){ $value = '00'.$v;
	}else if($v < 100000){ $value = '0'.$v;
	}else{ $value = $v;
	}
	return $value;
}
